...
def parse_arguments():
    ...
    parser.add_argument('--enable-wandb', type=str,
                        help='Enable Weights and Biases', default='disabled')
...
def prepare_data():
    df = pd.read_csv(f'{DATASET_DIR}/mtsamples.csv',
                     encoding='utf-8').iloc[:, 1:]
    df = df[['medical_specialty', 'description',
             'transcription', 'sample_name', 'keywords']]
    df['medical_specialty'] = df['medical_specialty'].str.strip()

    train_data, val_data = train_test_split(df.to_dict(
        'records'), stratify=df['medical_specialty'], test_size=0.1, random_state=42)
...
if __name__ == '__main__':
    ...
    CHECKPOINT_PATH = os.path.join(
        CHECKPOINT_DIR, get_model_name(args.model_name))
